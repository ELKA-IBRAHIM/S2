...
plt.xlabel("La fréquence en Khz")
plt.ylabel("Ondulation du courant en A")
plt.savefig('courbes/Ondulation en fct de la fréquence à rapport cyclique fixe.pdf')  # Save the figure as a pdf file
plt.show()
...
for j in range(len(alpha_list)):
    ...
    plt.grid()
    delta_i = alpha_j*(1-alpha_j)*Ve/(L*f_list)
    plt.plot(f_list/1e3, delta_i, "--", label = f"alpha = {alpha_j}")
